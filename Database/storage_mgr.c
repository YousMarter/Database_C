...
RC openPageFile(char *fileName, SM_FileHandle *fHandle){
    ...
    fHandle->mgmtInfo=(void*)RC_OPEN;
    //set the status of file, it must be open firstly if user wanna read or write
    //RC_OPEN presents the file has been check to be available to operate
    //so we just make sure it is RC_OPEN in read and write function
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage){
    ...
    FILE *file=fopen(fHandle->fileName,"r+");//open the file and it can be written
    int pos=pageNum*PAGE_SIZE;//compute expected value of position indicator
    fseek(file, pos, SEEK_SET);//move position indicator to expected position
    //if (ftell(file)==pos) {//check if indicator has been in expected position
...
RC appendEmptyBlock (SM_FileHandle *fHandle){
    ...
    return writeBlock(fHandle->totalNumPages, fHandle, newPage);
}
...
RC ensureCapacity (int numberOfPages, SM_FileHandle *fHandle){
    ...
    if(fHandle->totalNumPages<numberOfPages){
        for(i=fHandle->totalNumPages;i<numberOfPages+1;i++){
            RC wr=appendEmptyBlock(fHandle);
            if (wr!=RC_OK) {
                return wr;
            }
        }
    }//if the total number of pages is less than numberOfPages, then increase the pages to be equal to numberOfPages
    return RC_OK;
}